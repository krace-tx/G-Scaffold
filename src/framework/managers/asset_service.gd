...
func get_asset(id: StringName) -> Resource:
	if _cache.has(id):
		return _cache[id]
	if not Assets.has_id(id):
		App.log.error("asset", "unknown asset id: %s" % id)
		return null
	var res := load(Assets.load_path(id))
	_cache[id] = res
	return res
...
func preload_group(group: StringName) -> void:
	if group == &"":
		return
	var count := 0
	for id: StringName in Assets.ids_in_group(group):
		if not _cache.has(id):
			_cache[id] = load(Assets.load_path(id))
			count += 1
