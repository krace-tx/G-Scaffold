...
## 关卡系统、PrefabService 的职责。完整规格见 docs/modules/scene-service.md。
##
...
func reload_current() -> void:
	if _current_id == &"":
		App.log.warn("SceneService", "reload_current called with no current scene")
		return
	replace(_current_id)
...
func _fail(scene_id: StringName, reason: String) -> void:
	App.log.error("SceneService", "%s (id=%s)" % [reason, scene_id])
...
func _build_overlay() -> void:
	...
	layer.layer = 100   # 足够高,盖住普通游戏场景;后续 PrefabService 的层级需与此错开
