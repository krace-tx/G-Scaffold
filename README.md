...
- **业务全局聚合中枢**：通过全局单例 `Game` 统一收敛全局设置、远端动态配置与玩家数据档案；
...
└──────────────────┘          └──────────────────┘          └──────────────────┘
                                       ...
                                       ▼
                           ┌────────────────────────┐
                           │      Global Game       │  (Autoload / Business Root)
                           └───────────┬────────────┘
                                       │
         ┌─────────────────────────────┼─────────────────────────────┐
         ▼                             ▼                             ▼
┌──────────────────┐          ┌──────────────────┐          ┌──────────────────┐
│  Game.setting    │          │   Game.config    │          │   Game.profile   │
│  - Music / SFX   │          │  - Remote Config │          │  - User Profile  │
│  - Vibration     │          │  - Fallback Disk │          │  - Main Progress │
│  - Auto Persist  │          │  - Auto Refresh  │          │  - Dynamic Sync  │
└──────────────────┘          └──────────────────┘          └──────────────────┘
```
...
`App` 是框架层的全局服务定位器（Autoload: `res://src/framework/autoloads/app.gd`），统一承载 11 大核心底座能力：
...
`Game` 是业务层的全局聚合根单例（Autoload: `res://src/game/autoloads/game.gd`），用于集中管理游戏业务维度的核心状态与数据中枢：

| 业务中枢单例 | 功能与职责 |
| :--- | :--- |
| **`Game.setting`** | **全局音画与硬件设置中枢 (`SettingManager`)**<br>管理背景音乐（Music）、音效（SFX）、触觉震动（Vibrate）开关；支持状态变更事件派发与本地持久化自动同步。 |
| **`Game.config`** | **远端动态配置管理器 (`ConfigManager`)**<br>管理游戏全局策划配置（`GameConfig`）；支持从服务端 API 拉取最新配置、网络异常时自动降级读取本地缓存并容灾兜底。 |
| **`Game.profile`** | **玩家档案数据管理器 (`ProfileManager`)**<br>管理玩家的基础档案（`UserProfile`），包括用户 ID、昵称、头像与主线关卡进度等；支持深拷贝隔离与本地 JSON 自动化安全存盘。 |
