...
@onready var _tree: ResourceGroupTree = %GroupTree
...
var _tree_controller := ResourceGroupTreeController.new()
var _detail_presenter := ResourceGroupDetailPresenter.new()
...
func _setup_dialogs() -> void:
	...
	_new_group_dialog.title = "New resource group"
...
func _connect_signals() -> void:
	...
	_detail_presenter.resource_id_edited.connect(_on_resource_id_edited)
	_detail_presenter.resource_path_edited.connect(_on_resource_path_edited)
	_detail_presenter.resource_group_edited.connect(_on_resource_group_edited)
	_detail_presenter.resource_path_picked.connect(_on_resource_path_picked)
...
func _on_selection_changed(snapshot: ResourceGroupSelection) -> void:
	_detail_presenter.render(snapshot)


func _on_group_remove_requested(group_name: StringName) -> void:
	var result := ResourceGroupManifestOps.remove_group(_manifest, group_name)
	_apply_result(result)
	if result.is_ok():
		_commit_change()


func _on_items_reassigned(res_ids: Array[StringName], target_group: StringName) -> void:
	var result := ResourceGroupManifestOps.reassign_resources(_manifest, res_ids, target_group)
	_apply_result(result)
	if result.is_ok():
		var snapshot := _tree_controller.selection()
		snapshot.resource_ids = res_ids.duplicate()
		snapshot.resource_id = &"" if res_ids.size() != 1 else res_ids[0]
	_commit_change()
...
func _on_group_context_action(group_name: String, action_id: int) -> void:
	match action_id:
		...
		1:
			var result := ResourceGroupManifestOps.remove_group(_manifest, StringName(group_name))
			_apply_result(result)
			if result.is_ok():
				_commit_change()
...
func _on_add_id_requested(group_context: String) -> void:
	var result := ResourceGroupManifestOps.add_resource(_manifest, group_context)
	_apply_result(result)
	if result.is_err():
		return
	var entry: EditResourceEntry = result.value
	_tree_controller.selection().resource_id = entry.id
	_commit_change()
...
func _on_new_group_confirmed() -> void:
	var result := ResourceGroupManifestOps.add_group(_manifest, _new_group_edit.text)
	_apply_result(result)
	if result.is_ok():
		_commit_change()
...
func _apply_group_rename(old_name: StringName, raw_new_name: String) -> void:
	var result := ResourceGroupManifestOps.rename_group(_manifest, old_name, raw_new_name)
	_apply_result(result)
	if result.is_err():
...
func _on_resource_id_edited(res_id: StringName, new_text: String) -> void:
	var result := ResourceGroupManifestOps.update_resource_id(_manifest, res_id, new_text)
	_apply_result(result)
	if result.is_err():
		_detail_presenter.show_resource(res_id)
		return
	var new_id: StringName = result.value
	_tree_controller.sync_resource_tree_item(new_id)
	_detail_presenter.show_resource(new_id)
	changed.emit()


func _on_resource_path_edited(res_id: StringName, path: String) -> void:
	var result := ResourceGroupManifestOps.update_resource_path(_manifest, res_id, path)
	_apply_result(result)
	if result.is_ok():
		changed.emit()


func _on_resource_group_edited(res_id: StringName, group_name: String) -> void:
	var result := ResourceGroupManifestOps.set_resource_group(_manifest, res_id, group_name)
	_apply_result(result)
	if result.is_ok():
		_commit_change()


func _on_resource_path_picked(res_id: StringName, path: String) -> void:
	var result := ResourceGroupManifestOps.apply_path_pick(_manifest, res_id, path)
	_apply_result(result)
	if result.is_err():
		return
	var entry: EditResourceEntry = result.value
	_tree_controller.sync_resource_tree_item(entry.id)
	_detail_presenter.show_resource(entry.id)
	changed.emit()


func _on_remove_requested() -> void:
	var snapshot := _tree_controller.selection()
	var ids := snapshot.resource_ids.duplicate()
	if ids.is_empty() and snapshot.resource_id != &"":
		ids = [snapshot.resource_id]
	var result := ResourceGroupManifestOps.remove_resources(_manifest, ids)
	_apply_result(result)
	if result.is_ok():
		_commit_change()
