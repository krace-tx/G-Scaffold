@tool
class_name ResourceGroupTree
extends Tree

signal reassign_requested(resource_ids: Array[StringName], group_name: StringName)
...
func _build_drag_data(item: TreeItem) -> Variant:
	if item == null:
		return null
	var resource_ids := _collect_drag_resource_ids(item)
...
func _make_drag_preview(item: TreeItem, data: Dictionary) -> Control:
	...
	var resource_ids: Array = data["resource_ids"]
	if resource_ids.size() > 1:
		preview.text = "%d resources" % resource_ids.size()
	else:
...
func _can_drop_data(at_position: Vector2, data: Variant) -> bool:
	if typeof(data) != TYPE_DICTIONARY or not data.has("resource_ids"):
		return false
...
func _drop_data(at_position: Vector2, data: Variant) -> void:
	...
	if item == null:
		return
	reassign_requested.emit(data["resource_ids"], _group_name_from_item(item))


func _collect_drag_resource_ids(anchor_item: TreeItem) -> Array[StringName]:
